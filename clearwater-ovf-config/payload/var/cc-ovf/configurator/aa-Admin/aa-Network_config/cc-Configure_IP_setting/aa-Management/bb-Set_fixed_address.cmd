#!/bin/bash
#? Set the fixed IP for the management interface
#+ cc_admin
...
while [ $cmd_done -eq 0 ]; do
    ...
    cfrm=$?
    if [ $cfrm -eq 1 ]; then
	sed -ie '/^mgmt_fixed_address=/d' /var/lib/cc-ovf/configurator.wip; rm -f /var/lib/cc-ovf/configurator.wipe
	printf "mgmt_fixed_address=\"${new_VALUE}\"\n" >> /var/lib/cc-ovf/configurator.wip
	let "cmd_done=1"
    fi
