#!/bin/bash
#? Set the router(s) for the management interface
#+ cc_admin
...
new_VALUE=${mgmt_routers}

let "cmd_done=0"
while [ $cmd_done -eq 0 ]; do
    let "prompt_done=0"
    while [ $prompt_done -eq 0 ]; do
	doPrompt "Set the router(s) for the management interface\nSpecify the new set of routers" "" "$new_VALUE"
	let "prompt_done=1"
    ...
    cfrm=$?
    if [ $cfrm -eq 1 ]; then
	sed -ie '/^mgmt_routers=/d' /var/lib/cc-ovf/configurator.wip; rm -f /var/lib/cc-ovf/configurator.wipe
	printf "mgmt_routers=\"${new_VALUE}\"\n" >> /var/lib/cc-ovf/configurator.wip
	let "cmd_done=1"
    fi
