#!/bin/bash
#? Set the router(s) for the signaling interface
#+ cc_admin
...
while [ $cmd_done -eq 0 ]; do
    ...
    cfrm=$?
    if [ $cfrm -eq 1 ]; then
	sed -ie '/^sig_routers=/d' /var/lib/cc-ovf/configurator.wip; rm -f /var/lib/cc-ovf/configurator.wipe
	printf "sig_routers=\"${new_VALUE}\"\n" >> /var/lib/cc-ovf/configurator.wip
	let "cmd_done=1"
    fi
