#!/bin/bash
#? Set the DNS(s) for the management interface
#+ cc_admin
...
while [ $cmd_done -eq 0 ]; do
    ...
    cfrm=$?
    if [ $cfrm -eq 1 ]; then
	sed -ie '/^mgmt_domain_name_servers=/d' /var/lib/cc-ovf/configurator.wip; rm -f /var/lib/cc-ovf/configurator.wipe
	printf "mgmt_domain_name_servers=\"${new_VALUE}\"\n" >> /var/lib/cc-ovf/configurator.wip
	let "cmd_done=1"
    fi
